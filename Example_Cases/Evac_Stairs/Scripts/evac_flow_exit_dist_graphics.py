...
def plot_linear_model(m):
    ...
    """
    exit_distance = pl.arange(0., 700, 1)
    for theta_t in m.theta.trace():
        flow = theta_t[0]*exit_distance + theta_t[1]
        pl.plot(exit_distance, flow,
                color='gray', alpha=.75, zorder=-1)

    theta = m.theta.stats()['mean']
    flow = theta[0]*exit_distance + theta[1]
    pl.plot(exit_distance, flow,
...
def plot_power_law_model(m,
    ...
    """
    exit_distance = pl.arange(0.01, 700, 1)
    flow_trace = []
    ...
    pl.plot(exit_distance, pl.mean(flow_trace, axis=0),
...
def decorate_plot():
    ...
    pl.xlabel('Exit Distance (m)', fontsize=24)
